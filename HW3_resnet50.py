# -*- coding: utf-8 -*-
"""「ML2023-HW3-ImageClassification」resnet50

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1sQ4Ygit90A4-ihHPPsiONkSxuq5QtdEn
...
#引用pytorch官方套件from torchvision import models as M
import numpy as np
import pandas as pd
import torch
import os
...
#Transformer層的參數參考官方提供，並測試加入不同function所產生的效果
...
#存入每個checkpoint防止模型參數遺失，也可以節省運算單元的使用
def save_checkpoint(state, filename="/content/drive/MyDrive/Colab Notebooks/resnet50_170.pth.tar"):
    print("=> Saving checkpoint")
    torch.save(state, filename)
#寫出兩個checkpoint使用的function
...
old_checkpoint = torch.load("/content/drive/MyDrive/Colab Notebooks/resnet50_155.pth.tar")
...
for epoch in range(n_epochs):
    ...
    if valid_acc > best_acc:
        ...
        if stale > patience:
            ...
            break
#continue_epochs為第一階段訓練結束後還預計跑幾個epoch，而第一階段的模型參數會存入checkpoint
continue_epochs = 3
# 先讀取先前checkpoint的 model state
old_checkpoint = torch.load("/content/drive/MyDrive/Colab Notebooks/checkpoint.pth.tar")
...
df.to_csv("submission_50_170epoch.csv",index = False)

"""Voting for different(epoch) csv files"""

import numpy as np
import pandas as pd
pred1 = pd.read_csv("submission_50_100epoch.csv")
pred2 = pd.read_csv("submission_50_170epoch.csv")
pred3 = pd.read_csv("submission_50_155epoch.csv")
for idx, classes in enumerate(range(3000)):
    class1 = pred1.values[idx, 1]
    class2 = pred2.values[idx, 1]
    class3 = pred1.values[idx, 1]
    all_classes = np.array([class1, class2,class3])
    counts = np.bincount(all_classes)
    pred1.values[idx, 1] = np.argmax(counts)
pred1.to_csv("submission_final(100+170+155).csv", index=False)
